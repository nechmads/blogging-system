...
- [x] Multi-User — Phase 3: Data Migration — Auto-migrates 'default' seed user data to real Clerk user on first sign-in. DAL method `migrateDefaultUser()` updates all FK references (sessions, publications, social_connections) and the users PK in a single batch. Conditional WHERE with subquery count guard closes TOCTOU race window. Result verification via `meta.changes`. ensureUser middleware triggers migration transparently before falling back to normal createUser.
