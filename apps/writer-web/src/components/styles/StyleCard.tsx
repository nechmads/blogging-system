    ...
    <div className="rounded-xl border border-[var(--color-border-default)] bg-[var(--color-bg-primary)] p-4">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          ...
          {style.description && (
            <p className="mt-1 text-xs text-[var(--color-text-muted)]">{style.description}</p>
          )}
        </div>

        {style.isPrebuilt && (
          <span className="shrink-0 rounded-full bg-[var(--color-accent-light)] px-2 py-0.5 text-[10px] font-medium text-[var(--color-accent)]">
            Built-in
          </span>
        )}
      </div>
      ...
      {style.sampleText && (
        <blockquote className="mt-3 border-l-2 border-[var(--color-border-default)] pl-3 text-xs italic text-[var(--color-text-muted)]">
      ...
      <div className="mt-3 flex items-center gap-2">
        ...
        {onDuplicate && (
          <button
            ...
            className="flex items-center gap-1.5 rounded-lg border border-[var(--color-border-default)] px-3 py-1.5 text-xs font-medium text-[var(--color-text-muted)] transition-colors hover:bg-[var(--color-bg-card)]"
          >
            <CopyIcon size={12} />
            Duplicate
          </button>
        ...
        {onDelete && !style.isPrebuilt && (
          <button
            ...
            className="ml-auto flex items-center gap-1.5 rounded-lg border border-[var(--color-border-default)] px-3 py-1.5 text-xs font-medium text-[var(--color-text-muted)] transition-colors hover:bg-red-50 hover:text-red-500"
          >
            <TrashIcon size={12} />
            Delete
          </button>
