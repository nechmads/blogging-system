 ...
 *
 * Admin credentials:
 * Email: shahar@farfarawaylabs.com
...
async function seed() {
  ...
  try {
    ...
    const existingUser = await db
      .select()
      .from(users)
      .where(eq(users.email, 'shahar@farfarawaylabs.com'))
      .get()

    if (existingUser) {
      console.log('✓ Admin user already exists')
      console.log(`  Email: shahar@farfarawaylabs.com`)
      console.log(`  Role: ${existingUser.role}`)
      return
    }

    // Hash password using SHA-256 (same as SonicJS auth system)
    const data = 'BSuperman26!' + 'salt-change-in-production'
