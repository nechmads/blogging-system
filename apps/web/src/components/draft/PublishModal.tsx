...
export function PublishModal({ isOpen, onClose, sessionId, draftTitle, draftVersion, featuredImageUrl, sessionPublicationId, cmsPostId, onPublished, isRepublish }: PublishModalProps) {
  ...
  useEffect(() => {
    ...
    AnalyticsManager.track(AnalyticsEvent.PublishModalOpened, { isUpdate })
    ...
    if (isUpdate && cmsPostId) {
      ...
      generateSeo(sessionId)
        .then((seo) => {
          ...
          AnalyticsManager.track(AnalyticsEvent.SeoAutoGenerated)
        })
        .catch(() => { /* best-effort */ })
    ...
    generateTweet(sessionId, hookText)
      .then(({ tweet }) => {
        if (tweet) {
          setTweetText(tweet)
          setTweetFromAI(true)
          AnalyticsManager.track(AnalyticsEvent.TweetAutoGenerated)
        }
      })
      .catch(() => {
    ...
    generateLinkedInPost(sessionId, {
      ...
      .then(({ linkedInPost }) => {
        if (linkedInPost) {
          ...
          AnalyticsManager.track(AnalyticsEvent.LinkedInOptimized, { postMode: linkedInPostType })
        }
      })
      .catch(() => {
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      AnalyticsManager.track(AnalyticsEvent.PostPublished, {
        publicationId: selectedPubId ?? '',
        isUpdate,
        hasLinkedIn: linkedinSelected,
        hasTwitter: twitterSelected,
        hasFeaturedImage: !!featuredImageUrl,
      })
      // Track individual social shares
      if (linkedinSelected) {
        const liResult = result.socialResults?.find((sr) => sr.platform === 'linkedin')
        if (liResult?.success) {
          AnalyticsManager.track(AnalyticsEvent.LinkedInPostPublished, { publicationId: selectedPubId ?? '', postMode: linkedInPostType, usedAiOptimize: linkedInFromAI })
        }
      }
      if (twitterSelected) {
        const twResult = result.socialResults?.find((sr) => sr.platform === 'twitter')
        if (twResult?.success) {
          AnalyticsManager.track(AnalyticsEvent.TwitterPostPublished, { publicationId: selectedPubId ?? '', usedAiGenerate: tweetFromAI, tweetLength: effectiveTweetLength })
        }
      }
    } catch (err) {
