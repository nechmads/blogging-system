...
Ask focused questions, one or two at a time. Don't overwhelm with a long questionnaire. Build on what the user tells you. Once you have enough to work with, transition to drafting.`,
  ...
  researching: `You are in research mode. Use the search_web and lookup_source tools to find relevant information, data, and sources to support the post. Share what you find with the user and ask if the direction is right before proceeding to draft.`,
...
- **search_web**: Use when the user wants research-backed content or you need to verify facts. Currently returns placeholder results.
- **lookup_source**: Use when the user provides a specific URL to reference. Currently returns placeholder results.`
